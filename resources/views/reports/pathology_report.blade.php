...
<head>
    <meta charset="utf-8">
    <title>Pathology Report - {{ $visit->visit_number }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <div>
            ...
            <img src="{{ public_path('logo.png') }}" alt="Lab Logo" class="logo">
        </div>
    </div>
    ...
    <table class="patient-table">
        <tr>
            ...
            <td>{{ $visit->patient->birth_date ? $visit->patient->birth_date->age : '-' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>{{ $visit->referred_doctor ?? 'N/A' }}</td>
        </tr>
    </table>
    ...
    <div class="section-box">
        {{ $visit->recommendations ?? $visit->notes ?? '---' }}
    </div>

    <div class="barcode-block">
        @if(isset($barcode_base64))
            <img src="data:image/png;base64,{{ $barcode_base64 }}" alt="Sample Barcode" style="max-width:320px;" />
        @else
            {{-- Optionally generate barcode here if not passed from controller --}}
        @endif
        <div class="credentials">
            <strong>Patient Portal Login:</strong><br>
            Username: {{ $visit->patient->user->name ?? 'N/A' }}<br>
    ...
    <div class="signature-block">
        <div class="signature">
            ...
            <span style="font-size:0.95rem;">Date: {{ date('M d, Y') }}</span>
        </div>
    </div>
